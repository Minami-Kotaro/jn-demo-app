...
export const authorizationCheck : AuthorizationChecker = (action: Action):boolean => {
  try {
    //Bare {token}
...
export const currentUserCheck : CurrentUserChecker = (action: Action): AccountModel|undefined => {
  try {
    //Bare {token}
    const [_, token] = action.request.headers["authorization"].split(" ");
    const decodedToken = jwt.verify(token, tokenPassword, {
