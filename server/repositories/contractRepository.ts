import { ContractModel, InsertUpdateContractModel } from "../models/contractModel";
...
export async function GetNextContractId():Promise<number>{
  ...
  const result = await query.query("select nextval('seq_contract_id')").then(data =>
    Number(data[0].nextval))
...
export async function SelectContract(contractId:number):Promise<ContractModel | undefined> {
  const contract = await contractRepository.findOne({relations: ["user","contractManager"],
  ...
  return {
    ...
    contractManagerMailAddress:contract.contractManager.contractManagerMailAddress,
...
export async function InsertContracts(contract:InsertUpdateContractModel):Promise<void> {
...
export async function UpdateContract(contract:InsertUpdateContractModel):Promise<void>{
  const updateContract = await contractRepository.findOne({where: {contractId:contract.contractId}})
